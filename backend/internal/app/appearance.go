...
const (
	defaultAppearanceBrandName = "小卡"
	defaultAppearanceBrandSlug = "xiaoka"
	defaultAppearanceSkinID    = "classic"
	defaultAppearanceLogoURL   = "/logo.png"
...
func (s *Service) ResetAppearance(actor *model.User) (*AdminAppearanceSetting, error) {
	...
	if err := s.appendAdminAudit(actor, "appearance.reset", "system_setting", appearanceSettingKey, "恢复小卡默认品牌标识", map[string]any{"before": before, "after": after}); err != nil {
		return nil, err
	}
	return s.AdminAppearance(actor)
}
