...
if (Test-CanvasHttp $healthUrl) {
    Write-Host "小卡画布已经在运行，正在打开..." -ForegroundColor Green
...
if ($occupiedPorts.Count -gt 0) {
    throw "端口 $($occupiedPorts -join ', ') 已被其他程序占用，无法安全启动小卡。"
}
...
Write-Host "正在启动小卡前端和后端，请稍候..." -ForegroundColor Cyan
& $localStarter
...
throw "小卡服务已拉起，但 3 分钟内未通过健康检查。请查看打开的前端、后端窗口。"
